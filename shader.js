...
/* window.innerWidth > window.innerHeight
    ? Math.max(window.devicePixelRatio, 1.0)
    : 1.0;*/
...
// ocean blue, like blue forest wallpaper
/*let colors = [
  // "03045e",
  "023e8a",
  "0077b6",
...
colors = colors.concat(colors.slice(1, -1).reverse());
...
const aaf = 0.25; // anti-aliasing factor (textures will be of canvas size * aaf)
...
void main() {
  ...
  for (int i = 0; i < ${N}; i++) {
    ...
    if (value < (float(i) + 1.5) / ${N.toFixed(2)} /* u_colorStops[i] */) {
      color = u_colors[i];
      break;
    }
  }
...
var renderShaderSource = `
  ...
  4
      ...
      return ${
        colorBlendMode === "sharp"
          ? `texture2D(u_texture2, vec2((0.25 + floor(x * ${(
              N * edgeSmoothness
            ).toFixed(2)})) / ${(N * edgeSmoothness).toFixed(2)}, 0.5))`
          : `texture2D(u_texture2, vec2(x, 0.5))`
      };
    }

    // Combine textures using some operations
    vec4 combineTextures(vec2 st) {
        vec2 texelSize = ${ssRadius.toFixed(2)} / u_texture_resolution;
        vec4 tex1 = texture2D(u_texture1, st);
    ...
    void main() {
      ...
      gl_FragColor = color; // HueShift(color.rgb, ${Math.random()} + u_time * 0.001);
  }
`;
...
const antialiasingCode = `
   // vec4 tex12 = texture2D(u_texture1, vec2(st.x + texelSize.x, st.y - texelSize.y));
       // vec4 tex13 = texture2D(u_texture1, vec2(st.x - texelSize.x, st.y - texelSize.y));
       // vec4 tex14 = texture2D(u_texture1, vec2(st.x - texelSize.x, st.y + texelSize.y));
        // vec4 tex15 = texture2D(u_texture1, vec2(st.x + texelSize.x, st.y + texelSize.y));

        // uncomment to get additional antialiasing samples
      //  vec4 tex22 = sampleColorFromPalette(tex12);
      //  vec4 tex23 = sampleColorFromPalette(tex13);
      //  vec4 tex24 = sampleColorFromPalette(tex14);
       // vec4 tex25 = sampleColorFromPalette(tex15);
        // vec4 res = tex2 * ${(1.0 - ssStrength).toFixed(2)} + tex22 * ${(
  ssStrength / 4.0
).toFixed(2)} + tex23 * ${(ssStrength / 4.0).toFixed(2)} + tex24 * ${(
  ssStrength / 4.0
).toFixed(2)} + tex25 * ${(ssStrength / 4.0).toFixed(2)};
        // vec4 res = tex2 * ${(1.0 - ssStrength).toFixed(
          2
        )} + tex23 * ${ssStrength.toFixed(2)};

        // vec4 res = tex2 * ${(1.0 - ssStrength).toFixed(2)} + tex23 * ${(
  ssStrength / 2.0
).toFixed(2)} + tex25 * ${(ssStrength / 2.0).toFixed(2)};
...
canvas.width = window.innerWidth * dpr * aaf;
canvas.height = window.innerHeight * dpr * aaf;
...
function renderLoop(time) {
  ...
  // limit fps
  if (delta < 1000.0 / fpsLimit) return;
  prevTime = time;
  ...
  gl.uniform1f(renderTimeLocation, time * 0.01); // Pass time in seconds
